...
// ─── STONE PAYMENT ────────────────────────────────────────────────────────────
function StoneModal({ total, onClose, onSuccess }) {
  ...
  function pay() {
    ...
    const t = setInterval(() => {
      p += Math.random() * 15 + 6;
      if (p >= 100) { p = 100; clearInterval(t); setTimeout(() => setStep("success"), 300); }
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"Georgia,serif",background:C.offWhite,minHeight:"100vh",color:C.dark }}>
      ...
      {showPay&&<StoneModal total={cart.reduce((s,i)=>s+i.price,0)} onClose={()=>setShowPay(false)} onSuccess={()=>{setShowPay(false);setCart([]);}} />}
